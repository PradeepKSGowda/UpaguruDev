...
* `reviewed_by` (UUID, Nullable -> Auth User)
* `pdf_document_id` (UUID, Foreign Key -> `pdf_documents.id`, Nullable)
* `run_id` (UUID, Foreign Key -> `crawl_runs.id`, Nullable)
* `raw_text` (Text, Nullable)
* `parent_draft_id` (UUID, Foreign Key -> `draft_notifications.id`, Nullable)
* `is_corrigendum` (Boolean, Default: false)
* `extraction_model` (Text, Nullable)
* `prompt_version` (Text, Nullable)
* `completeness_score` (Numeric(5, 4), Nullable)
...
### 4. `pdf_documents`
* `id` (UUID, Primary Key)
* `sha256_hash` (Text, Unique Index)
* `source_portal` (Text - e.g., KPSC, UPSC, SSC, RRB)
* `source_url` (Text)
...
### 5. `crawl_runs`
* `id` (UUID, Primary Key)
* `portal_code` (Text - e.g., KPSC, UPSC, SSC, RRB)
