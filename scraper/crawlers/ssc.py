"""SSC (Staff Selection Commission) portal crawler."""
...
class SSCCrawler(BaseCrawler):
    """Crawler for Staff Selection Commission (ssc.gov.in)."""

    def __init__(self, config: Optional[dict[str, Any]] = None, fetcher: Optional[BaseFetcher] = None):
        cfg = config or load_portal_config("SSC")
        if fetcher is None:
            if cfg.get("engine") == "httpx":
                fetcher = HttpxFetcher(timeout_seconds=cfg.get("timeout_seconds", 45))
            else:
    ...
    def _matches_keywords(self, text: str) -> bool:
        """Filter notifications to retain only notices of examination and recruitment."""
    ...
    def _parse_notification_links(self, html: str) -> list[dict[str, str]]:
        """Extract PDF URLs, notice titles, and publication dates from SSC DOM."""
        soup = BeautifulSoup(html, "html.parser")
        base_url = self.config.get("base_url", "https://ssc.gov.in")
        found_items: list[dict[str, str]] = []

        rows = soup.select("table tbody tr, .notice-item, .card-body tr, ul.list-group li")
        logger.info("Found candidate DOM rows for SSC", count=len(rows))

        for row in rows:
            pdf_link_elem = row.select_one("a[href*='.pdf'], a[href*='.PDF'], a[href*='download'], a[href*='blob']")
            ...
            if not self._matches_keywords(title):
                logger.debug("Skipping non-target SSC notice", title=title[:60])
                continue

            cells = row.select("td")
            pub_date = cells[0].get_text(strip=True) if len(cells) > 1 else None
        ...
        logger.info("Extracted SSC PDF items", count=len(found_items), portal=self.portal_code)
        return found_items

    async def crawl(self) -> CrawlResult:
        """Execute complete SSC crawl cycle."""
        target_url = self.config.get("notifications_url", "https://ssc.gov.in/notices")
        ...
        try:
            ...
            logger.info("Started SSC crawl run", run_id=run_id)
        ...
        try:
            ...
            for item in detected_items:
                ...
                try:
                    ...
                    if downloaded.is_duplicate:
                        ...
                        result.items.append(
                            CrawledNotificationItem(
                                title=title,
                                pdf_url=pdf_url,
                                source_portal=self.config.get("portal_name", "SSC"),
                    ...
                    try:
                        ...
                        logger.info("Inserted new SSC pdf_document", hash=downloaded.sha256_hash)
                    except Exception as exc:
                    ...
                    result.failed += 1
                    error_msg = f"Failed processing SSC PDF {pdf_url}: {exc}"
            ...
            logger.error("SSC crawl run failed", error=str(exc))
